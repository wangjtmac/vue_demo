...
export default {
    ...
    marksData() {
      ...
      data.forEach((item) => {
        ...
        for (let i = 0; i < item.number; i++) {
          dataList.push({
            value: [
              this.randomNum(points.x[0], points.x[1]),
              this.randomNum(points.y[0], points.y[1])
            ]
          })
        }
      })
  ...
  methods: {
    ...
    getRandomRange(condition) {
      let result = `${condition.includes('不说') ? '0' : '1'}${
        condition.includes('不做') ? '0' : '1'
      }`
