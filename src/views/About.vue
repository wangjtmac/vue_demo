    ...
    <div class="about">
        ...
        <common-table :data="tableData" :columns="tableHead">
            ...
            <template slot="img" slot-scope="{row , $index}">
                <div v-if="row.img">
                    <img v-for="(img , i) in row.img" :key="i" height="40" class="ml5" :src="img" alt="">
                </div>
            </template>
        </common-table>
        ...
        >
            <el-form size="mini" label-position="right" label-width="80px">
                <el-form-item v-for="(item , key) in formList" :key="key">
                    ...
                    <el-input v-if="item.type === 'input-number'" v-input-limit:number v-model.number="form[key]"></el-input>
                    <el-select class="width100" v-if="item.type === 'select'" v-model="form[key]">
        ...
        <div class="ce-box"
             v-ce-loading:[loadingOption]="loading">
        </div>
...
export default {
    ...
    methods: {
        getPostDetail(){
            const vm = this, {postId , service} = vm;
        ...
        submit(){
            const vm = this, {form , service , editState} = vm;
            if(editState === "new"){
