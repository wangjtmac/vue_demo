  ...
  methods: {
    startResize (event) {
      ...
      const drag = (event) => {
        ...
        var splitSize = h
          ? ((event.x - parentRect.left - splitter) / this.$el.clientWidth) *
            100
          : ((event.y - parentRect.top - splitter) / this.$el.clientHeight) *
            100
  ...
  render (h) {
    ...
    items.push(
      h(
        ...
        {
          class: 'content',
          attrs: { style: 'flex-basis:' + this.state.split }
        },
        [this.$slots.default[0]]
      )
    )
    items.push(
      h('div', { class: 'splitter', on: { mousedown: this.startResize } })
    )
